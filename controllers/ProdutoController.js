...
module.exports = {
    ...
    async store(req, res) {
        ...
        if(tipo_produto_id < 1 || tipo_produto_id > 7){
            res.status(400).json({ erro: `Tipo de produto inválido, favor informar um tipo entre 1 e 7`});
            return;
        }
    ...
    async update(req, res) {
        ...
        try {
            const data = await knex('produtos').where('id', id).update({ nome, valor, foto });
            if (data)
    ...
    async destroy(req, res) {
        ...
        try {
            const data = await knex('produtos').where('id', id).del();
            if (data)
